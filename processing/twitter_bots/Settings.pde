  ...
  Settings(String _s) {
    try {
      ...
      for (int i=0;i<data.length;i++) {
        if (data[i].equals("Consumer Key")) cKey = setString(data[i+1]);
        if (data[i].equals("Consumer Secret")) cSecret = setString(data[i+1]);
        if (data[i].equals("Access Token")) aToken = setString(data[i+1]);
        if (data[i].equals("Access Token Secret")) aTokenSecret = setString(data[i+1]);
